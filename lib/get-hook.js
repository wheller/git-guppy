...
function getGitCdPath() {
  ...
  if(typeof process.env.GIT_PREFIX === 'string' && process.env.GIT_PREFIX.trim()) {
    // in git bash on windows, the separator is \ but the git environment variables use / so they must be normalized
    gitRootCdPath = path.normalize(process.env.GIT_PREFIX).replace(new RegExp('[^'+dirsep+']+','g'),'..');
